...
import { loadLogo, drawLogo } from "./pdfLogo";
...
export async function buildInvoicePdf({ settings, invoice, job, items }) {
  ...
  const doc = new jsPDF();
  const logo = await loadLogo("/logo.png", LOGO_W);
  let y = 20;

  // ---- Logo, top right ----
  drawLogo(doc, logo, RIGHT - LOGO_W, 14, LOGO_W, LOGO_W * LOGO_RATIO);
  ...
  // Addressed to whoever pays. When a company is on the customer record, the
  // invoice is made out to the company and the person becomes the contact —
  // a business can't claim GST on an invoice made out to an employee.
  const cust = job?.customers || {};
  const billedTo = String(cust.company_name ?? "").trim();
  if (billedTo) {
    doc.text("Customer: " + billedTo, LEFT, y); y += 5;
    doc.text("Attn: " + (cust.name || ""), LEFT, y); y += 5;
  } else {
    doc.text("Customer: " + (cust.name || ""), LEFT, y); y += 5;
  }

  // Their address, phone and email, indented under the name. Two reasons this is
  // not optional: a tax invoice over $1,000 has to carry the buyer's name AND
  // address, and a printed invoice is often the only thing in the customer's
  // hand — a name on its own is no use to whoever files it.
  //
  // Anything blank is left out rather than printed as an empty line, so a
  // customer with no address on file still gets a tidy invoice. The address is
  // one free-text field, so it is split on its own line breaks and each line is
  // wrapped rather than running off the right-hand edge.
  const CUST_X = LEFT + 4;      // indent, so the detail sits under the name
  const CUST_W = 90;            // wrap width, well clear of the totals column
  for (const raw of String(cust.address ?? "").split(/\r?\n/)) {
    const line = raw.trim();
    if (!line) continue;
    for (const part of doc.splitTextToSize(line, CUST_W)) { doc.text(part, CUST_X, y); y += 5; }
  }
  const phone = String(cust.phone ?? "").trim();
  const email = String(cust.email ?? "").trim();
  if (phone) { doc.text("Ph: " + phone, CUST_X, y); y += 5; }
  if (email) { doc.text(email, CUST_X, y); y += 5; }
  // Rent and hireage invoices have no machine — the line is left off entirely
  // rather than printed empty.
  const machineLine = [job?.machines?.type, job?.machines?.make, job?.machines?.model].filter(Boolean).join(" ");
  if (machineLine) {
    doc.text("Machine: " + machineLine, LEFT, y);
    y += 10;
  } else {
    y += 4;
  }
