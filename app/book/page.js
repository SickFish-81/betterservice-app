...
// Public booking form. The only thing this page can do is call
// submit_booking_request() — it cannot read any table, and the request lands in
// a holding pen for staff to accept. See migration 0047.
...
export default function BookPage() {
  ...
  async function submit(e) {
    ...
    const { error } = await supabase.rpc("submit_booking_request", {
      ...
      p_notes: f.notes || null, p_trap: f.trap || null,
    });
    setBusy(false);
    ...
    return (
      <main className="mx-auto max-w-lg px-4 py-16">
        <div className="rounded-xl border border-green-200 bg-green-50 p-6">
          <h1 className="text-xl font-bold text-green-900">Thanks — that&apos;s booked in.</h1>
  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-10">
      <h1 className="text-3xl font-bold tracking-tight text-zinc-900">Book a service</h1>
      ...
      <form onSubmit={submit} className="mt-6 flex flex-col gap-5">
        ...
        <fieldset className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          <legend className="px-1 text-sm font-semibold text-zinc-900">The machine</legend>
          ...
          <div className="mt-3">
            <label htmlFor="problem" className={label}>What needs doing?</label>
            <textarea id="problem" value={f.problem} onChange={set("problem")} rows={3} required placeholder="A service, or what's gone wrong" className={input} />
          </div>
        </fieldset>
