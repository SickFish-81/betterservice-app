...
export default function StaffPage() {
  ...
  // Save an edited phone number for an existing staff member (on blur).
  async function savePhone(s, value) {
    const next = value.trim();
    if ((s.phone || "") === next) return;
    const { error } = await supabase.from("staff").update({ phone: next || null }).eq("id", s.id);
    if (error) { setError(error.message); return; }
    setStaff((prev) => prev.map((x) => (x.id === s.id ? { ...x, phone: next || null } : x)));
  }
            ...
            {staff.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 p-4">
                ...
                <span className="flex items-center gap-2 text-sm text-zinc-500">
                  <input defaultValue={s.phone || ""} onBlur={(e) => savePhone(s, e.target.value)} type="tel" placeholder="Phone" className="w-32 rounded-lg border border-zinc-200 px-2 py-1 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-red-500 focus:outline-none" />
