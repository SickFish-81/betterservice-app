...
const LEFT = 20;
const RIGHT = 190;
const BOTTOM = 258;          // start a new page below this — leaves room for the footer block
...
export async function buildInvoicePdf({ settings, invoice, job, items }) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    terms.forEach((ln, k) => doc.text(ln, LEFT, PAGE_H - 26 + k * 3.2));

    // Hairline, then the strapline.
    doc.setDrawColor(210);
    doc.line(LEFT, PAGE_H - 16, RIGHT, PAGE_H - 16);
    doc.setDrawColor(0);

    doc.setFontSize(9);
    doc.text(TAGLINE, PAGE_W / 2, PAGE_H - 11, { align: "center" });
    if (pages > 1) {
      doc.text(`Page ${i} of ${pages}`, RIGHT, PAGE_H - 11, { align: "right" });
    }
