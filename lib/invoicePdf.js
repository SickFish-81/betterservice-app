...
const LEFT = 20;
const RIGHT = 190;
const BOTTOM = 275;          // start a new page below this
...
export async function buildInvoicePdf({ settings, invoice, job, items }) {
  ...
  doc.setFontSize(18);
  doc.text(settings?.business_name || "Betterservice Te Puke", LEFT, y);
  y += 7;
  ...
  const DESC_W = 120;           // mm of description column before the amount
  const LINE_H = 5;
  ...
  (items || []).forEach((it) => {
    ...
    const qty = `(${it.quantity} x $${Number(it.unit_price).toFixed(2)})`;
    const lines = doc.splitTextToSize(label, DESC_W);

    // Keep a line item and its quantity note together on one page.
    if (y + (lines.length + 1) * LINE_H > BOTTOM) newPage();

    // Amount sits on the first line, description flows below it.
